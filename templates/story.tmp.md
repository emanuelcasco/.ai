...
__Remove this section before using the template__

- If no issue manager is specified (like Linear, Jira, etc) then simply generate the issue number as `#` in the @~/.ai/stories folder (create it if necessary).

- If issue manager is specified (like Linear, Jira, etc) then use the issue number from the issue manager or create a new issue based on the user input. Ask for any extra details if needed.

- For the [Dev Agent Record](#dev-agent-record) section:
  - If working on a local file, simply append the records to the file.
  - If working with a issue manager, create a comment whenever possible to the issue.
    - Sign the issue with the dev agent name.
